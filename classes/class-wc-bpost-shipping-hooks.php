...
class WC_BPost_Shipping_Hooks {
	...
	/**
	 * In the plugins list, the links below the plugin name
	 */
	 ...
	 */
	public function bpost_shipping_add_method( array $methods ): array {
	...
	/**
	 * Admin: In the orders list, add a bulk action
	 */
	public function bpost_shipping_add_order_bulk_action( array $actions ): array {
	...
	private function get_wc_order( $post_or_order ): WC_Order {
		if ( $post_or_order instanceof WC_Order ) {
			return $post_or_order;
		}

		if ( $order = wc_get_order( $post_or_order ) ) {
			// Compatibility with WP Overnight plugin
			// @see https://wordpress.org/support/topic/fatal-error-wc_bpost_shipping_hooksget_wc_order-return-value-must-be-of-ty/
		...
		throw new InvalidArgumentException( 'Cannot retrieve WC_Order from argument' );
	}
	...
	public function bpost_shipping_info_block( WC_Order $order ) {
		...
		$order_display = new WC_BPost_Shipping_Order_Details_Controller(
			...
			new WC_BPost_Shipping_Meta_Handler( Container::get_meta_type(), $order ),
...
'
	...
	public function bpost_shipping_api_loader() {
		$posted_obj           = WC_BPost_Shipping_Posted::create( $_POST );
		...
		$data_builder = new WC_BPost_Shipping_Data_Builder(
			...
			new WC_BPost_Shipping_Delivery_Methods(
				Container::get_api_connector()->fetchProductConfig()
			)
		);
	...
	private function is_bpost_shipping_from_session(): bool {
			...
			switch ( $shipping_method ) {
				...
				case BPOST_PLUGIN_ID . '_error':
					Container::get_logger()->notice(
						'It is a bpost shipping', [ 'chosen_shipping_methods' => $chosen_shipping_methods ]
					);
		...
		Container::get_logger()->notice(
			'It is not a bpost shipping', [ 'chosen_shipping_methods' => $chosen_shipping_methods ]
		);
	...
	private function is_bpost_shipping_from_order( WC_Order $order ): bool {
	 ...
	 */
	public function bpost_order_details_box_meta( $post_type, $order ) {
		if ( ! in_array( $post_type, [ 'shop_order', 'woocommerce_page_wc-orders' ] ) ) {
			return;
		}

		if ( ! ( $order instanceof WC_Order || $order instanceof WP_Post ) ) {
			return;
		}
	 ...
	 * @return void
	 */
	public function bpost_order_details_box_meta_add( $order ) {
		if ( $order instanceof WP_Post ) {
			$order = wc_get_order( $order );
		}
		$adapter      = Container::get_adapter();
		$meta_handler = new \WC_BPost_Shipping_Meta_Handler( new \WC_BPost_Shipping_Meta_Type( $adapter ), $order );
		...
		$label_meta_box_controller = new WC_BPost_Shipping_Label_Meta_Box_Controller(
			...
			new WC_BPost_Shipping_Label_Attachment(
				...
				new WC_BPost_Shipping_Label_Post( $meta_handler, $order )
			)
		);
	...
	/**
	 * Admin: Add "Print bpost label" at each bpost order line
	 */
	public function bpost_order_review_admin_actions( $actions, WC_Order $the_order ): array {
		if ( ! $this->is_bpost_shipping_from_order( $the_order ) ) {
			return $actions;
		}

		$meta_handler = new \WC_BPost_Shipping_Meta_Handler( Container::get_meta_type(), $the_order );
...
HTML;
	...
	public function bpost_shipping_options_validation() {
		...
		Container::get_order_review()->review_order( WC_BPost_Shipping_Posted::create( $posted_data ) );
