...
class WC_BPost_Shipping_Limitations {
	...
	private WC_BPost_Shipping_Api_Connector $connector;
	 ...
	 * @return bool
	 */
	public function validate_limitations( $payment_method, $address, $weight ) {
		...
		if ( ! $this->is_valid_weight( $this->product_configuration, $weight ) ) {
			$this->errors[]     = sprintf(
				bpost__( 'Shipments over %s kg are not possible with bpost, the current cart weight is %s kg' ),
				number_format( $this->product_configuration->get_maximal_allowed_weight() / 1000, 2 ),
				number_format( $weight, 2 )
			);
			$limitations_are_ok = false;
		}

		if ( ! $this->check_webservice_status( $this->connector ) ) {
			$this->errors[]     =
