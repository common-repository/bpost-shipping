...
class WC_BPost_Shipping_Assets_Resources {
	...
	public function get_order_receive_page( $order_received ) {
		wp_enqueue_script( 'bpost-shm-app', BPOST_PLUGIN_URL . 'public/js/app.min.js', array(), BPOST_PLUGIN_VERSION );
		wp_enqueue_style( 'checkout_css_file',
			BPOST_PLUGIN_URL . 'public/css/checkout.min.css',
			array(),
			BPOST_PLUGIN_VERSION );
		wp_register_script( 'bpost-order-received',
			BPOST_PLUGIN_URL . 'public/js/order-received.min.js',
			array(),
			BPOST_PLUGIN_VERSION );
	...
	public function get_admin_bpost_page() {
		wp_enqueue_style( 'font_awesome',
			BPOST_PLUGIN_URL . 'public/css/font-awesome.min.css',
			array(),
			BPOST_PLUGIN_VERSION );
		wp_enqueue_style( 'admin_css_file',
			BPOST_PLUGIN_URL . 'public/css/admin.min.css',
			array( 'font_awesome' ),
			BPOST_PLUGIN_VERSION );
	}
	...
	public function get_admin_order_edit_page( array $order_data ) {
		...
		wp_register_script( 'bpost-admin-order-edit',
			BPOST_PLUGIN_URL . 'public/js/admin.min.js',
			array(),
			BPOST_PLUGIN_VERSION );
	 ...
	 */
	public function get_callback_page( array $shm_data ) {
		header( 'Content-Type: text/html' ); // force the content-type. On some cases, the content-type was text/xml
		wp_enqueue_script( 'bpost-shm-app', BPOST_PLUGIN_URL . 'public/js/app.min.js', array(), BPOST_PLUGIN_VERSION );
